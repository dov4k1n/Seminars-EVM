...
void check_solution(double **B, int m, int n, double *root) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    long double difference = fabs(sum - B[i][n - 1]);
    printf("row %d:\t %Le\n", i + 1, difference);
  }
  printf("\n");
}
...
int main() {
  ...
  if (load_matrix(A, in, m, n) || load_matrix(B, in, m, n)) {
    for (i = 0; i < m; i++)
      free(A[i]);
    free(A);
    for (i = 0; i < m; i++)
  ...
  printf("\nnullspace dimension = %d, nullspace basis:\n", n - 1 - rank);
